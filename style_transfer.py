...
"""this file contains the style transfer model. Only small changes,
as change of the optimizer and the weights; 
Style transfer mostly taken from: https://pytorch.org/tutorials/advanced/neural_style_tutorial.html
...
def get_input_optimizer(input_img):
    ...
    #optimizer = optim.SGD([input_img.requires_grad_()], lr=0.00001, momentum=0.95)
...
def apply_style(content_latent, style_latent, mvc_model, patt_model, num_iter=0, title='styleTransfer_interpolation', styleTransfer=True):
    """Method that prepares the images for the colour-based style transfer. Converts the image tensors into cv2-images 
       and applies the color_segmentation method which returns the segmented image based on hsv color filters.
         ----------
    ...
    result = perform_style_transfer(content_image, content_image_tensor, style_image, style_image_tensor, skin_seg, back_seg, num_iter, title, styleTransfer)
    return result

    
def perform_style_transfer(content_image, content_img_tensor, style_image, style_img_tensor, skin_seg ,back_seg, num_iter, title='styleTransfer_interpolation', styleTransfer=True):
    """Method that uses the segmented images, applies some morphological transformation to smoothen the edges and
       calls the style transfer forward pass. After that, segmented image and image with style transfer are combined
       such that only the garment is styled.
         ----------
        content_image, content_image_tensor: cv2 and pytorch tensor representation of the content image (garment)
        style_image, style_image_tensor: cv2 and pytorch tensor representation of the style image (pattern)
        skin_seg, back_seg: segmented images (one color-space for skin and one for background)
        num_iter: current iteration of the interpolation
        styleTransfer: boolean variable is style transfer is carried out or not
    """
    ...
    #apply the style transfer or simply apply the pattern image to the garment
    if(styleTransfer):
        output = run_style_transfer(model.features.cuda().eval(), cnn_normalization_mean, cnn_normalization_std,
                                        content_img_tensor, style_img_tensor, input_img, num_steps=10000)
    else:
        output = style_img_tensor
        
    #convert tensor into numpy uint array
    foreground_img = cv2.normalize(output.squeeze(0).permute(1,2,0).detach().cpu().numpy(), None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
    foreground_img = cv2.cvtColor(foreground_img, cv2.COLOR_BGR2RGB)
    
    if(not styleTransfer):
        cv2.imwrite('./output/foreground_no_style_transfer.jpg',foreground_img)
    ...
    cv2.imwrite('./output/{}_{}.jpg'.format(title, num_iter),back_seg)
...
def color_segmentation(image): 
    """Method that segments an image based on one color space for skin and one color space for the background. 
       Values have been set by manual analysis.
         ----------
        image: cv2 image that should be segmented
    """
